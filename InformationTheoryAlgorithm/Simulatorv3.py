...
# Given a word str it multiplies its priority by a constant
def set_word_priority(word, mult=0.6):
    word_priority[get_position_of_word(word)] *= mult
...
@cache
def make_guess(wordlist, mode, removed):
    ...
    for i,w in enumerate(patterns):
        ...
        if mode == 'greedy1' or mode == 'greedy3':
            e *= get_word_priority(wordlist[i])
        if e > maxe and wordlist[i] not in removed:
            maxe = e
            best = i
...
def play(solution, allowed=words, removed = {}, starter='tares', mode='naive'):
    ...
    for attempt in range(9):
        ...
        if pattern == 242:
            #if mode == 'greedy3' or mode == 'greedy4':
                #set_word_priority(next, 0.9)
            break
        ...
        i, next = make_guess(tuple(remaining_words), mode, tuple(removed))
...
def simulations(mode, setstarter = None):
    ...
    allowed = words
    if mode == 'greedy2' or mode == 'greedy4':
        allowed = solutions
    removed = []
    results = [0 for _ in range(9)]
    solutionset = np.array(solutions)
    shuffle(solutionset)
    for solution in tqdm(solutionset):
        results[play(solution, allowed, removed, starter, mode)] += 1
        if mode == 'greedy3' or mode == 'greedy4':
...
# Greedy2 : Relaxation problem 1
# Greedy3 : Relaxation Problem 2
modes = ['naive','greedy1','greedy2','greedy3', 'greedy4']
while True:
    mode = input('Choose mode: '+str(modes)+'\n')
